...
class ShoppingTableViewController: UITableViewController,FoodShopDelegate {
    ...
    /*
        This function is to perform AddShopSegue.
     */
    @objc func addSegue() {
        if !self.checkWiFi() {
            let alert = UIAlertController(title: "Disconnection", message: "Your device is disconnected.\r\nplease try to login again", preferredStyle: .alert)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let addBarButtonItem = UIBarButtonItem(title: "Add", style: .plain, target: self, action: #selector(addSegue))
        self.navigationItem.rightBarButtonItem  = addBarButtonItem
        addBarButtonItem.tintColor = UIColor.darkGray
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if showList!.count == 0 {
            ...
            default:
                ...
                case 0:
                    ...
                    cell.cellBody?.text = "Tap Add Button below to add your plan"
        ...
        case -1:
            ...
            cell.tagImage.image = UIImage(named: "right-blue-50px")
        default:
            cell.tickImage.image = UIImage(named: "grey-tick-52px")
            cell.circleImage.image = UIImage(named: "grey-circle-50px")
            cell.tagImage.image = UIImage(named: "right-50px")
        }
    ...
    func updateCompletion(id:Int,completion:Int) {
        ...
        guard let putUrl = URL(string: HttpUrl().updateFoodCompletion + "?" + dataString) else { return}
    ...
    func updateFoodPrice(foodId:Int,price:String) {
        ...
        guard let putUrl = URL(string:HttpUrl().updateFoodPrice + "?" + dataString!) else { return}
        var putRequest = URLRequest(url: putUrl)
        putRequest.httpMethod = "PATCH"
        putRequest.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let data = dataString!.data(using: String.Encoding.utf8)
    ...
    func updateFood(foodId:Int,foodName:String,foodBuy:String,foodExpire:String,foodPrice: String,category:String,location:String) {
        ...
        guard let putUrl = URL(string:HttpUrl().updateFoodById + "?" + dataString) else { return}
        var putRequest = URLRequest(url: putUrl)
        putRequest.httpMethod = "PATCH"
        putRequest.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let data = dataString.data(using: String.Encoding.utf8)
