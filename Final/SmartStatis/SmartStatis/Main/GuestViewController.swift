...
class GuestViewController: UIViewController,UITextFieldDelegate {
    ...
    func generateReference() {
        let loginURL = HttpUrl().getAllUserSQL
    ...
    func guestSignUp(reference: String) {
        guard let postUrl = URL(string: HttpUrl().addUserSQL + "?firstname="  + "guest" + "&lastname=" + "guest" + "&email=" + reference + "@hotmail.com" + "&password=" + "guest") else { return}
    ...
    func guestSignIn(reference:String) {
        let loginURL = HttpUrl().getUserByEmailAndPassword + "?email=" + reference + "@hotmail.com" + "&password=guest"
        guard let url = URL(string: loginURL) else { return}
        let session = URLSession.shared
        session.dataTask(with: url){(data,response,error) in
            if let data = data{
